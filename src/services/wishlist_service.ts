...
export const addToWishlist = async (userId: string, bookId: string) => {
  ...
  if (!existing) {
    await WishlistModel.dbCreate({ userId: userId, bookId: bookId });
  }
};
...
export const getWishlistBooks = async (userId: string): Promise<IBook[]> => {
  const wishlistItems = await WishlistModel.dbFind(userId);

  const bookIdArray = wishlistItems.map((item) => item.bookId);
